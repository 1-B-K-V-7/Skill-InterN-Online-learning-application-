...
class Trainer(models.Model):
    ...
    course = models.ForeignKey(Courses, on_delete=models.SET_NULL, null=True) #If we delete courses ..trainer is gonna get deleted

    def __str__(self):
        return self.trainer_name


class Register(models.Model):  # Register is database table name
    ...
    email = models.EmailField(unique=True)         # 1 course can be registered using only 1 email
    ...
    companyName = models.CharField(max_length=150, blank=True, null=True) #blank and null specifies this field can be left empty
    designation = models.CharField(max_length=150)
    qualification = models.CharField(max_length=100)
    computerKnowledge = models.CharField(max_length=50)
    Course = models.CharField(max_length=150)
    timestamp = models.DateField(auto_now_add=True)
    trainer = models.ForeignKey(Trainer, on_delete=models.DO_NOTHING, null=True)  # If we deletecTrainer ..registered student not  gonna get deleted
